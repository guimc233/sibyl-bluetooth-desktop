...
## 🎨 Windows 11 UWP 现代交互工作流 (v2.0 全新重构)

1. **两页式沉浸导航（Two-Page Navigation Workflow）**：
   * **页面 1【周边设备发现与连接页】**：打开应用即自动开启无感低功耗蓝牙扫描，以卡片形式动态列出周边所有 SIBYL 耳机，展示实时信号强度（RSSI）与 MAC 地址，点击即可一键建立 GATT 隔离连接。
   * **页面 2【耳机深度状态调整详情页】**：连接成功后平滑切入控制中心；左上角随附 UWP 风格返回键，可随时返回发现页或切换耳机。
2. **智能设备记忆与自动回连（Smart Device Memory & Auto-Reconnect）**：
   * 首次在列表中手动选择连接耳机后，App 会在本地安全记住该耳机标识（设备 ID 与名称）。
   * 下次打开应用时，无需重复点击，一旦蓝牙广播侦测到上次设备在附近，即刻**自动静默重连**并直达控制面板！
3. **两级降噪调节全面支持（Comfort / Deep Dual-Level ANC）**：
   * 针对绝大部分降噪机型（如 S1/S11/B6/B1/Y1/Y7/CH500/WF200 等），在开启主动降噪时提供【舒适降噪 (50%)】与【深度降噪 (100%)】两级快速切换，平衡耳压与静音需求。
4. **隐蔽式高级选项抽屉（Hidden Advanced Drawer）**：
   * 界面右上角配备齿轮按钮，点击从右侧滑出抽屉。
   * 将**手动修改兼容型号配置**、**驱动内核切换（真实 WinRT BLE / 模拟模式）**及**出厂重置**等低频开发运维选项全部收拢在抽屉内，保持主交互页面极简纯粹。
3. **免依赖自包含单文件（Self-Contained Executable）**：
   * 内置 .NET 8 核心运行时，无任何前置安装要求，双击即可直接运行；自带调试控制台窗口，启动与蓝牙交互日志全程可视化，彻底告别静默闪退。
