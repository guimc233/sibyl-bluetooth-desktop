...
## 🎨 WinUI 3 (WinUI-Gallery 规范) 现代交互工作流 (v2.2 全新升级)

1. **正统 WinUI-Gallery 控件与排版规范**：
   * 采用微软官方 [WinUI-Gallery](https://github.com/Microsoft/WinUI-Gallery) 规范设计系统：深色 Mica 分层底色、8px 微圆角、半透明亚克力卡片、Accent 强调色按钮与 Subtle 辅助按钮，现代感十足。
2. **两页式沉浸导航（Two-Page Navigation Workflow）**：
   * **页面 1【周边设备发现与连接页】**：打开应用即自动开启无感低功耗蓝牙扫描，以 WinUI 卡片形式动态列出周边设备。
   * **SIBYL 官方耳机智能判定认证**：集成与官方 Android 端一致的广播识别算法（Service UUID + 厂商自定义字段 `0x3E21/0x12CC` + 名称过滤），自动标注“SIBYL 官方认证”绿色徽章并优先排列，过滤无关蓝牙设备！
   * **页面 2【耳机深度状态调整详情页】**：点击连接后无缝平滑切入控制中心；左上角随附 WinUI 导航返回键，随时返回并切换设备。
3. **两级降噪调节全面打通（舒适降噪 50% / 深度降噪 100%）**：
   * 全系 ANC 降噪机型支持两档消噪深度一键调节，兼顾室内低耳压与喧闹场景极致静谧。
4. **智能设备记忆与自动回连（Smart Device Memory & Auto-Reconnect）**：
   * 首次连接成功后自动安全记住该耳机标识；下次启动应用侦测到在线即刻**自动静默重连**直达控制台！
5. **隐蔽式高级选项抽屉（Hidden Advanced Drawer）**：
   * 界面右上角配备齿轮按钮，滑出侧边抽屉，收纳硬件型号兼容覆盖、BLE 驱动内核切换及出厂重置，界面干净纯粹。
6. **免依赖自包含单文件（Self-Contained Executable）**：
   * 内置 .NET 8 核心运行时，开箱双击即用，控制台窗口实时可视化蓝牙收发报文日志。
